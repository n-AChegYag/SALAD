...
class IpNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1):
        ...
        self.down6 = Down(512, 512, dropout=0.5)
        self.down7 = Down(512, 512, dropout=0.5)
        self.down8 = Down(512, 512, normalize=False, dropout=0.5)
...
class FpNet(nn.Module):
    ...
    def forward(self, z):
        ...
        z = self.up6(z)         # 32 -> 64
        z = self.up7(z)         # 64 -> 128
        x_p = self.up8(z)       # 128 -> 256

        z = self.down1(x_p)     # 256 -> 128
        z = self.down2(z)       # 128 -> 64
        z = self.down3(z)       # 64 -> 32
        z = self.down4(z)       # 32 -> 16
        z = self.down5(z)       # 16 -> 8
        z = self.down6(z)       # 8 -> 4
        z = self.down7(z)       # 4 -> 2
        z = self.down8(z)     # 2 -> 1
